...
{
    public partial class AdministrarRegla : GenericPage
    {
        ...
        protected void gvDxCatalogo_RowCommand(object sender, DevExpress.Web.ASPxGridView.ASPxGridViewRowCommandEventArgs e)
        {
            ...
            if (e.CommandArgs.CommandName.Equals("Remove"))
            {
                ...
                try
                {
                    Regla removeRegla = (Regla)gvDxCatalogo.GetRow(e.VisibleIndex);
                    ServiceLocator.Instance.GetService<ReglaManager>().Remove(removeRegla);
                    ...
                    if (ex.InnerException != null)
                    {
                        if (ex.InnerException.InnerException != null)
                        {
                            ...
                            if (excepcionIBM.Errors[0].SQLState.Equals("23000") || excepcionIBM.Errors[0].SQLState.Equals("23001"))
                            {
                                ...
                                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "key", "<script type=\"text/javascript\"> refreshRegla(); </script>", false);
            ...
            else
            {
                ...
                {
                    Regla editRegla = (Regla)gvDxCatalogo.GetRow(e.VisibleIndex);
                    Page.Session.Add("EditReglaCatalogoID", editRegla.ReglaId);

                    Response.Redirect("/Process/Catalogos/Registro/NuevoRegla.aspx");
